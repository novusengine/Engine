...
public:
  ...
  /// Find all within search rectangle
  /// \param a_center Min of search bounding rect
  /// \param a_radius Max of search bounding rect
  /// \param a_searchResult Search result array.  Caller should set grow size. Function will reset, not append to array.
  /// \param a_resultCallback Callback function to return result.  Callback should return 'true' to continue searching
  /// \param a_context User context to pass as parameter to a_resultCallback
  /// \return Returns the number of entries found
  int SearchRadius(const ELEMTYPE a_center[NUMDIMS], const ELEMTYPE a_radius, std::function<bool(const DATATYPE&)> callback) const;
...
protected:
  ...
  bool OverlapRadius(Sphere* a_sphere, Rect* a_rect) const;
  ...
  bool SearchRadius(Node* a_node, Sphere* a_sphere, int& a_foundCount, std::function<bool (const DATATYPE&)> callback) const;
...
RTREE_TEMPLATE
int RTREE_QUAL::SearchRadius(const ELEMTYPE a_center[NUMDIMS], const ELEMTYPE a_radius, std::function<bool(const DATATYPE&)> callback) const
{
#ifdef _DEBUG
    RTREE_ASSERT(a_radius >= 0);
#endif //_DEBUG

  Sphere sphere;
  for(int axis=0; axis<NUMDIMS; ++axis)
  {
    sphere.m_center[axis] = a_center[axis];
  }
  sphere.m_radius = a_radius;

  // NOTE: May want to return search result another way, perhaps returning the number of found elements here.

  int foundCount = 0;
  SearchRadius(m_root, &sphere, foundCount, callback);
...
// Decide whether a sphere and a rectangles overlap.
RTREE_TEMPLATE
bool RTREE_QUAL::OverlapRadius(Sphere* a_sphere, Rect* a_rect) const
{
    RTREE_ASSERT(a_sphere && a_rect);

    ELEMTYPE squared_distance = 0;
    ELEMTYPE closest_point[NUMDIMS];
    ELEMTYPE squared_radius = a_sphere->m_radius * a_sphere->m_radius; // Pre-calculate squared radius

    for (int index = 0; index < NUMDIMS; ++index)
    {
        // Find the closest point on the rectangle to the sphere center in this dimension
        if (a_sphere->m_center[index] < a_rect->m_min[index])
        {
            closest_point[index] = a_rect->m_min[index];
        }
        else if (a_sphere->m_center[index] > a_rect->m_max[index])
        {
            closest_point[index] = a_rect->m_max[index];
        }
        else
        {
            closest_point[index] = a_sphere->m_center[index];
        }

        // Calculate the squared distance in this dimension
        ELEMTYPE dimension_distance = a_sphere->m_center[index] - closest_point[index];
        squared_distance += dimension_distance * dimension_distance;
    }

    // Sphere and rectangle overlap if the squared distance to the closest point on the rectangle is less than or equal to the squared radius of the sphere
    return squared_distance <= squared_radius; // Use pre-calculated squared radius
}
...
// Search in an index tree or subtree for all data retangles that overlap the argument sphere.
RTREE_TEMPLATE
bool RTREE_QUAL::SearchRadius(Node* a_node, Sphere* a_sphere, int& a_foundCount, std::function<bool (const DATATYPE&)> callback) const
{
  RTREE_ASSERT(a_node);
  RTREE_ASSERT(a_node->m_level >= 0);
  RTREE_ASSERT(a_sphere);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(OverlapRadius(a_sphere, &a_node->m_branch[index].m_rect))
      {
        if(!SearchRadius(a_node->m_branch[index].m_child, a_sphere, a_foundCount, callback))
        {
          // The callback indicated to stop searching
          return false;
        }
