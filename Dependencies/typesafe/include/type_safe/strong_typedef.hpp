...
#define STRONG_TYPEDEF(declName, underlyingType) \
struct declName : type_safe::strong_typedef<declName, underlyingType>, \
type_safe::strong_typedef_op::integer_arithmetic<declName>, \
type_safe::strong_typedef_op::equality_comparison<declName>, \
type_safe::strong_typedef_op::relational_comparison<declName> \
{ \
	using strong_typedef::strong_typedef; \
    using type = underlyingType; \
    static underlyingType MaxValue() \
    { \
        return std::numeric_limits<underlyingType>::max(); \
    } \
    static underlyingType MaxValueTyped() \
    { \
        return (underlyingType)std::numeric_limits<underlyingType>::max(); \
    } \
    static declName Invalid() \
    { \
        return declName(declName(declName::MaxValueTyped())); \
    } \
    declName() \
    { \
        value_ = (underlyingType)Invalid(); \
    } \
};
