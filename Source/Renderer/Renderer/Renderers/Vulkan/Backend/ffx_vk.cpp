...
namespace Renderer
{
    ...
    namespace Backend
    {
        ...
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->GetSDKVersionVK(backendInterface);
        }

        FfxErrorCode CreateBackendContextVK(FfxInterface* backendInterface, FfxUInt32* effectContextId)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->CreateBackendContextVK(backendInterface, effectContextId);
        }

        FfxErrorCode GetDeviceCapabilitiesVK(FfxInterface* backendInterface, FfxDeviceCapabilities* deviceCapabilities)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->GetDeviceCapabilitiesVK(backendInterface, deviceCapabilities);
        }

        FfxErrorCode DestroyBackendContextVK(FfxInterface* backendInterface, FfxUInt32 effectContextId)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->DestroyBackendContextVK(backendInterface, effectContextId);
        }
        ...
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->CreateResourceVK(backendInterface, desc, effectContextId, outTexture);
        }

        FfxErrorCode DestroyResourceVK(FfxInterface* backendInterface, FfxResourceInternal resource)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->DestroyResourceVK(backendInterface, resource);
        }
        ...
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->RegisterResourceVK(backendInterface, inResource, effectContextId, outResourceInternal);
        }

        FfxResource GetResourceVK(FfxInterface* backendInterface, FfxResourceInternal resource)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->GetResourceVK(backendInterface, resource);
        }

        FfxErrorCode UnregisterResourcesVK(FfxInterface* backendInterface, FfxCommandList commandList, FfxUInt32 effectContextId)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->UnregisterResourcesVK(backendInterface, commandList, effectContextId);
        }

        FfxResourceDescription GetResourceDescriptionVK(FfxInterface* backendInterface, FfxResourceInternal resource)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->GetResourceDescriptionVK(backendInterface, resource);
        }
        ...
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->CreatePipelineVK(backendInterface, effect, passId, permutationOptions, desc, effectContextId, outPass);
        }

        FfxErrorCode DestroyPipelineVK(FfxInterface* backendInterface, FfxPipelineState* pipeline, FfxUInt32 effectContextId)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->DestroyPipelineVK(backendInterface, pipeline, effectContextId);
        }

        FfxErrorCode ScheduleGpuJobVK(FfxInterface* backendInterface, const FfxGpuJobDescription* job)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->ScheduleGpuJobVK(backendInterface, job);
        }

        FfxErrorCode ExecuteGpuJobsVK(FfxInterface* backendInterface, FfxCommandList commandList)
        {
            FidelityFXHandlerVK* fidelityFXHandlerVK = reinterpret_cast<FidelityFXHandlerVK*>(backendInterface->device);
            return fidelityFXHandlerVK->ExecuteGpuJobsVK(backendInterface, commandList);
        }
    }
    ...
    namespace Backend
    {
        ...
        static u32 _maxEffectContexts = 8;
        ...
        FfxSurfaceFormat ffxGetSurfaceFormatFromVKSurfaceFormat(VkFormat fmt)
        {
            switch (fmt)
            {
        ...
        void FidelityFXHandlerVK::Init(RenderDeviceVK* device, ImageHandlerVK* imageHandler)
        {
            _device = device;
            _imageHandler = imageHandler;
        }

        size_t FidelityFXHandlerVK::ffxGetScratchMemorySize(size_t maxContexts)
        {
            uint32_t numExtensions = 0;

            if (_device->_physicalDevice)
                vkEnumerateDeviceExtensionProperties(_device->_physicalDevice, nullptr, &numExtensions, nullptr);
        ...
        FfxResource FidelityFXHandlerVK::ffxGetResource(ImageID imageID, FfxResourceStates state)
        {
            const ImageDesc& desc = _imageHandler->GetImageDesc(imageID);
            uvec2 dimensions = _imageHandler->GetDimensions(imageID, 0);

            FfxResource resource = {};
            resource.resource = (void*)_imageHandler->GetImage(imageID);
            resource.state = state;

            resource.description.flags = FFX_RESOURCE_FLAGS_NONE;
            resource.description.usage = (FfxResourceUsage)(FFX_RESOURCE_USAGE_READ_ONLY | FFX_RESOURCE_USAGE_UAV);
            resource.description.width = dimensions.x;
            resource.description.height = dimensions.y;
            resource.description.depth = 1;
            resource.description.mipCount = desc.mipLevels;
            resource.description.format = ffxGetSurfaceFormatFromVKSurfaceFormat(FormatConverterVK::ToVkFormat(desc.format));
            resource.description.type = FFX_RESOURCE_TYPE_TEXTURE2D;

            return resource;
        }

        FfxResource FidelityFXHandlerVK::ffxGetResource(DepthImageID imageID, FfxResourceStates state)
        {
            const DepthImageDesc& desc = _imageHandler->GetImageDesc(imageID);
            uvec2 dimensions = _imageHandler->GetDimensions(imageID);
            ...
            resource.description.flags = FFX_RESOURCE_FLAGS_NONE;
            resource.description.usage = (FfxResourceUsage)(FFX_RESOURCE_USAGE_DEPTHTARGET | FFX_RESOURCE_USAGE_UAV);
            resource.description.width = dimensions.x;
            resource.description.height = dimensions.y;
            resource.description.depth = 1;
        ...
        FfxUInt32 FidelityFXHandlerVK::GetSDKVersionVK(FfxInterface* backendInterface)
        {
            return FFX_SDK_MAKE_VERSION(FFX_SDK_VERSION_MAJOR, FFX_SDK_VERSION_MINOR, FFX_SDK_VERSION_PATCH);
        }

        FfxErrorCode FidelityFXHandlerVK::CreateBackendContextVK(FfxInterface* backendInterface, FfxUInt32* effectContextId)
        {
            ...
            if (!_backendRefCount)
            {
                ...
                uint32_t gpuJobDescArraySize = FFX_ALIGN_UP(_maxEffectContexts * FFX_MAX_GPU_JOBS * sizeof(FfxGpuJobDescription), sizeof(uint32_t));
                uint32_t resourceViewArraySize = FFX_ALIGN_UP(_maxEffectContexts * FFX_MAX_QUEUED_FRAMES * FFX_MAX_RESOURCE_COUNT * 2 * sizeof(BackendContext_VK::VkResourceView), sizeof(uint32_t));
                uint32_t ringBufferArraySize = FFX_ALIGN_UP(_maxEffectContexts * FFX_RING_BUFFER_SIZE * sizeof(BackendContext_VK::UniformBuffer), sizeof(uint32_t));
                uint32_t pipelineArraySize = FFX_ALIGN_UP(_maxEffectContexts * FFX_MAX_PASS_COUNT * sizeof(BackendContext_VK::PipelineLayout), sizeof(uint32_t));
                uint32_t resourceArraySize = FFX_ALIGN_UP(_maxEffectContexts * FFX_MAX_RESOURCE_COUNT * sizeof(BackendContext_VK::Resource), sizeof(uint32_t));
                uint32_t contextArraySize = FFX_ALIGN_UP(_maxEffectContexts * sizeof(BackendContext_VK::EffectContext), sizeof(uint32_t));
                ...
                if (_vkDeviceProcAddr == NULL)
                {
                    _vkDeviceProcAddr = vkGetDeviceProcAddr;
                }

                if (_device->_device != VK_NULL_HANDLE) 
                {
                    backendContext->device = _device->_device;
                }

                if (_device->_physicalDevice != VK_NULL_HANDLE) 
                {
                    backendContext->physicalDevice = _device->_physicalDevice;
                }

                // load vulkan functions
                backendContext->vkFunctionTable.vkSetDebugUtilsObjectNameEXT = (PFN_vkSetDebugUtilsObjectNameEXT)_vkDeviceProcAddr(backendContext->device, "vkSetDebugUtilsObjectNameEXT");
                backendContext->vkFunctionTable.vkFlushMappedMemoryRanges = (PFN_vkFlushMappedMemoryRanges)_vkDeviceProcAddr(backendContext->device, "vkFlushMappedMemoryRanges");
                backendContext->vkFunctionTable.vkCreateDescriptorPool = (PFN_vkCreateDescriptorPool)_vkDeviceProcAddr(backendContext->device, "vkCreateDescriptorPool");
                backendContext->vkFunctionTable.vkCreateSampler = (PFN_vkCreateSampler)_vkDeviceProcAddr(backendContext->device, "vkCreateSampler");
                backendContext->vkFunctionTable.vkCreateDescriptorSetLayout = (PFN_vkCreateDescriptorSetLayout)_vkDeviceProcAddr(backendContext->device, "vkCreateDescriptorSetLayout");
                backendContext->vkFunctionTable.vkCreateBuffer = (PFN_vkCreateBuffer)_vkDeviceProcAddr(backendContext->device, "vkCreateBuffer");
                backendContext->vkFunctionTable.vkCreateImage = (PFN_vkCreateImage)_vkDeviceProcAddr(backendContext->device, "vkCreateImage");
                backendContext->vkFunctionTable.vkCreateImageView = (PFN_vkCreateImageView)_vkDeviceProcAddr(backendContext->device, "vkCreateImageView");
                backendContext->vkFunctionTable.vkCreateShaderModule = (PFN_vkCreateShaderModule)_vkDeviceProcAddr(backendContext->device, "vkCreateShaderModule");
                backendContext->vkFunctionTable.vkCreatePipelineLayout = (PFN_vkCreatePipelineLayout)_vkDeviceProcAddr(backendContext->device, "vkCreatePipelineLayout");
                backendContext->vkFunctionTable.vkCreateComputePipelines = (PFN_vkCreateComputePipelines)_vkDeviceProcAddr(backendContext->device, "vkCreateComputePipelines");
                backendContext->vkFunctionTable.vkDestroyPipelineLayout = (PFN_vkDestroyPipelineLayout)_vkDeviceProcAddr(backendContext->device, "vkDestroyPipelineLayout");
                backendContext->vkFunctionTable.vkDestroyPipeline = (PFN_vkDestroyPipeline)_vkDeviceProcAddr(backendContext->device, "vkDestroyPipeline");
                backendContext->vkFunctionTable.vkDestroyImage = (PFN_vkDestroyImage)_vkDeviceProcAddr(backendContext->device, "vkDestroyImage");
                backendContext->vkFunctionTable.vkDestroyImageView = (PFN_vkDestroyImageView)_vkDeviceProcAddr(backendContext->device, "vkDestroyImageView");
                backendContext->vkFunctionTable.vkDestroyBuffer = (PFN_vkDestroyBuffer)_vkDeviceProcAddr(backendContext->device, "vkDestroyBuffer");
                backendContext->vkFunctionTable.vkDestroyDescriptorSetLayout = (PFN_vkDestroyDescriptorSetLayout)_vkDeviceProcAddr(backendContext->device, "vkDestroyDescriptorSetLayout");
                backendContext->vkFunctionTable.vkDestroyDescriptorPool = (PFN_vkDestroyDescriptorPool)_vkDeviceProcAddr(backendContext->device, "vkDestroyDescriptorPool");
                backendContext->vkFunctionTable.vkDestroySampler = (PFN_vkDestroySampler)_vkDeviceProcAddr(backendContext->device, "vkDestroySampler");
                backendContext->vkFunctionTable.vkDestroyShaderModule = (PFN_vkDestroyShaderModule)_vkDeviceProcAddr(backendContext->device, "vkDestroyShaderModule");
                backendContext->vkFunctionTable.vkGetBufferMemoryRequirements = (PFN_vkGetBufferMemoryRequirements)_vkDeviceProcAddr(backendContext->device, "vkGetBufferMemoryRequirements");
                backendContext->vkFunctionTable.vkGetImageMemoryRequirements = (PFN_vkGetImageMemoryRequirements)_vkDeviceProcAddr(backendContext->device, "vkGetImageMemoryRequirements");
                backendContext->vkFunctionTable.vkAllocateDescriptorSets = (PFN_vkAllocateDescriptorSets)_vkDeviceProcAddr(backendContext->device, "vkAllocateDescriptorSets");
                backendContext->vkFunctionTable.vkAllocateMemory = (PFN_vkAllocateMemory)_vkDeviceProcAddr(backendContext->device, "vkAllocateMemory");
                backendContext->vkFunctionTable.vkFreeMemory = (PFN_vkFreeMemory)_vkDeviceProcAddr(backendContext->device, "vkFreeMemory");
                backendContext->vkFunctionTable.vkMapMemory = (PFN_vkMapMemory)_vkDeviceProcAddr(backendContext->device, "vkMapMemory");
                backendContext->vkFunctionTable.vkUnmapMemory = (PFN_vkUnmapMemory)_vkDeviceProcAddr(backendContext->device, "vkUnmapMemory");
                backendContext->vkFunctionTable.vkBindBufferMemory = (PFN_vkBindBufferMemory)_vkDeviceProcAddr(backendContext->device, "vkBindBufferMemory");
                backendContext->vkFunctionTable.vkBindImageMemory = (PFN_vkBindImageMemory)_vkDeviceProcAddr(backendContext->device, "vkBindImageMemory");
                backendContext->vkFunctionTable.vkUpdateDescriptorSets = (PFN_vkUpdateDescriptorSets)_vkDeviceProcAddr(backendContext->device, "vkUpdateDescriptorSets");
                backendContext->vkFunctionTable.vkCmdPipelineBarrier = (PFN_vkCmdPipelineBarrier)_vkDeviceProcAddr(backendContext->device, "vkCmdPipelineBarrier");
                backendContext->vkFunctionTable.vkCmdBindPipeline = (PFN_vkCmdBindPipeline)_vkDeviceProcAddr(backendContext->device, "vkCmdBindPipeline");
                backendContext->vkFunctionTable.vkCmdBindDescriptorSets = (PFN_vkCmdBindDescriptorSets)_vkDeviceProcAddr(backendContext->device, "vkCmdBindDescriptorSets");
                backendContext->vkFunctionTable.vkCmdDispatch = (PFN_vkCmdDispatch)_vkDeviceProcAddr(backendContext->device, "vkCmdDispatch");
                backendContext->vkFunctionTable.vkCmdDispatchIndirect = (PFN_vkCmdDispatchIndirect)_vkDeviceProcAddr(backendContext->device, "vkCmdDispatchIndirect");
                backendContext->vkFunctionTable.vkCmdCopyBuffer = (PFN_vkCmdCopyBuffer)_vkDeviceProcAddr(backendContext->device, "vkCmdCopyBuffer");
                backendContext->vkFunctionTable.vkCmdCopyImage = (PFN_vkCmdCopyImage)_vkDeviceProcAddr(backendContext->device, "vkCmdCopyImage");
                backendContext->vkFunctionTable.vkCmdCopyBufferToImage = (PFN_vkCmdCopyBufferToImage)_vkDeviceProcAddr(backendContext->device, "vkCmdCopyBufferToImage");
                backendContext->vkFunctionTable.vkCmdClearColorImage = (PFN_vkCmdClearColorImage)_vkDeviceProcAddr(backendContext->device, "vkCmdClearColorImage");
                backendContext->vkFunctionTable.vkCmdFillBuffer = (PFN_vkCmdFillBuffer)_vkDeviceProcAddr(backendContext->device, "vkCmdFillBuffer");
                ...
                {
                    { VK_DESCRIPTOR_TYPE_SAMPLER, _maxEffectContexts * FFX_MAX_RESOURCE_COUNT * FFX_MAX_PASS_COUNT * FFX_MAX_QUEUED_FRAMES },
                    { VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, _maxEffectContexts * FFX_MAX_RESOURCE_COUNT * FFX_MAX_PASS_COUNT * FFX_MAX_QUEUED_FRAMES },
                    { VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, _maxEffectContexts * FFX_MAX_RESOURCE_COUNT * FFX_MAX_PASS_COUNT * FFX_MAX_QUEUED_FRAMES },
                    { VK_DESCRIPTOR_TYPE_SAMPLER, _maxEffectContexts * FFX_MAX_RESOURCE_COUNT * FFX_MAX_PASS_COUNT * FFX_MAX_QUEUED_FRAMES  },
                    { VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, _maxEffectContexts * FFX_MAX_RESOURCE_COUNT * FFX_MAX_PASS_COUNT * FFX_MAX_QUEUED_FRAMES },
                };
                ...
                descriptorPoolCreateInfo.maxSets = _maxEffectContexts * FFX_MAX_PASS_COUNT * FFX_MAX_QUEUED_FRAMES;
                ...
                {
                    for (uint32_t i = 0; i < FFX_RING_BUFFER_SIZE * _maxEffectContexts; i++)
                    {
                    ...
                    for (uint32_t i = 0; i < _maxEffectContexts * FFX_RING_BUFFER_SIZE; i++)
                    {
                        BackendContext_VK::UniformBuffer& uBuffer = backendContext->pRingBuffer[i];
        ...
        FfxErrorCode FidelityFXHandlerVK::GetDeviceCapabilitiesVK(FfxInterface* backendInterface, FfxDeviceCapabilities* deviceCapabilities)
        {
            BackendContext_VK* backendContext = (BackendContext_VK*)backendInterface->scratchBuffer;
        ...
        FfxErrorCode FidelityFXHandlerVK::DestroyBackendContextVK(FfxInterface* backendInterface, FfxUInt32 effectContextId)
        {
        ...
        FfxErrorCode FidelityFXHandlerVK::CreateResourceVK(FfxInterface* backendInterface, const FfxCreateResourceDescription* createResourceDescription, FfxUInt32 effectContextId, FfxResourceInternal* outResource)
        {
            FFX_ASSERT(NULL != backendInterface);
        ...
        FfxErrorCode FidelityFXHandlerVK::DestroyResourceVK(FfxInterface* backendInterface, FfxResourceInternal resource)
        {
        ...
        FfxErrorCode FidelityFXHandlerVK::RegisterResourceVK(FfxInterface* backendInterface, const FfxResource* inFfxResource, FfxUInt32 effectContextId, FfxResourceInternal* outFfxResourceInternal)
        {
            FFX_ASSERT(NULL != backendInterface);
            BackendContext_VK* backendContext = (BackendContext_VK*)(backendInterface->scratchBuffer);
            BackendContext_VK::EffectContext& effectContext = backendContext->pEffectContexts[effectContextId];
        ...
        FfxResource FidelityFXHandlerVK::GetResourceVK(FfxInterface* backendInterface, FfxResourceInternal inResource)
        {
        ...
        FfxErrorCode FidelityFXHandlerVK::UnregisterResourcesVK(FfxInterface* backendInterface, FfxCommandList commandList, FfxUInt32 effectContextId)
        {
            FFX_ASSERT(NULL != backendInterface);
            BackendContext_VK* backendContext = (BackendContext_VK*)(backendInterface->scratchBuffer);
            BackendContext_VK::EffectContext& effectContext = backendContext->pEffectContexts[effectContextId];
        ...
        FfxResourceDescription FidelityFXHandlerVK::GetResourceDescriptionVK(FfxInterface* backendInterface, FfxResourceInternal resource)
        {
            FFX_ASSERT(NULL != backendInterface);
            BackendContext_VK* backendContext = (BackendContext_VK*)backendInterface->scratchBuffer;
        ...
        FfxErrorCode FidelityFXHandlerVK::CreatePipelineVK(FfxInterface* backendInterface, FfxEffect effect, FfxPass pass, uint32_t permutationOptions, const FfxPipelineDescription* pipelineDescription, FfxUInt32 effectContextId, FfxPipelineState* outPipeline)
        {
            FFX_ASSERT(NULL != backendInterface);
        ...
        FfxErrorCode FidelityFXHandlerVK::DestroyPipelineVK(FfxInterface* backendInterface, FfxPipelineState* pipeline, FfxUInt32 effectContextId)
        {
            FFX_ASSERT(backendInterface != nullptr);
            BackendContext_VK* backendContext = (BackendContext_VK*)backendInterface->scratchBuffer;
            BackendContext_VK::EffectContext& effectContext = backendContext->pEffectContexts[effectContextId];
        ...
        FfxErrorCode FidelityFXHandlerVK::ScheduleGpuJobVK(FfxInterface* backendInterface, const FfxGpuJobDescription* job)
        {
            FFX_ASSERT(NULL != backendInterface);
            FFX_ASSERT(NULL != job);
        ...
        static FfxErrorCode executeGpuJobCompute(BackendContext_VK* backendContext, FfxGpuJobDescription* job, VkCommandBuffer vkCommandBuffer)
        {
            ...
            for (uint32_t currentRootConstantIndex = 0; currentRootConstantIndex < job->computeJobDescriptor.pipeline.constCount; ++currentRootConstantIndex, ++bufferDescriptorIndex, ++descriptorWriteIndex)
            {
                ...
                if (backendContext->ringBufferBase >= FFX_RING_BUFFER_SIZE * _maxEffectContexts)
...
#define FFX_MAX_IMAGE_COPY_MIPS 14 // Will handle 4k down to 1x1
        ...
        FfxErrorCode FidelityFXHandlerVK::ExecuteGpuJobsVK(FfxInterface* backendInterface, FfxCommandList commandList)
        {
            FFX_ASSERT(NULL != backendInterface);
            BackendContext_VK* backendContext = (BackendContext_VK*)backendInterface->scratchBuffer;
