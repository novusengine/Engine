	...
	bool Chunk::FromADT(Adt::Layout& layout, Chunk& out)
	{
        ...
        for (u16 i = 0; i < Terrain::CHUNK_NUM_CELLS; i++)
        {
            ...
            for (u16 j = 0; j < Terrain::CELL_TOTAL_GRID_SIZE; j++)
            {
                ...
                if (height < out.heightHeader.gridMinHeight)
                    out.heightHeader.gridMinHeight = height;

                if (height > out.heightHeader.gridMaxHeight)
                    out.heightHeader.gridMaxHeight = height;
                ...
                cell.colorData[j][0] = cellInfo.mccv.color[j].red;
                cell.colorData[j][1] = cellInfo.mccv.color[j].green;
                cell.colorData[j][2] = cellInfo.mccv.color[j].blue;
            }
